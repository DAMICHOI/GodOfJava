...
- [Chapter 17. 어노테이션이라는 것도 알아야 한다](#Chapter-17-어노테이션이라는-것도-알아야-한다)
- [Chapter 18. 이제 기본 문법은 거의 다 배웠으니 정리해 봅시다](#Chapter-18-이제-기본-문법은-거의-다-배웠으니-정리해-봅시다)
- [<자바의 신> 중간 점검 및 실습](#자바의-신-중간-점검-및-실습)
...
1. 인터페이스에 선언되어 있는 메소드는 body(몸통)가 있어도 되나요?
    - 안된다.
    - 인터페이스에서는 메소드를 선언만 해야 한다.
    - 메소드 선언 후 중괄호를 열고 닫기만 해도 컴파일이 됮 ㅣ않는다.


2. 인터페이스를 구현하는 클래스의 선언 시 사용하는 예약어는 무엇인가요?
    - `implements`


3. 일부만 완성되어 있는 클래스를 무엇이라고 하나요?
    - 추상 클래스 (abstract)
    - abstract 클래스는 인터페이스처럼 메소드를 선언만 할 수도 있고, 일부 메소드를 구현할 수도 있다.


4. 3번의 답에 있는 클래스에 body(몸통)가 없는 메소드를 추가하려면 어떤 예약어를 추가해야 하나요?
    - `abstract` 예약어
    - 당연히 해당 클래스도 abstract class로 선언되어 있어야만 한다.


5. 클래스를 final로 선언하면 어떤 제약이 발생하나요?
    - 내용의 변경이 불가능하다.
    - 더 이상 확장(extends)이 불가능하다.


6. 메소드를 final로 선언하면 어떤 제약이 발생하나요?
    - 메소드를 final로 선언하면 더 이상 Override 할 수 없다.


7. 변수를 final로 선언하면 어떤 제약이 발생하나요?
    - 변수의 값을 변경할 수 없다.
    - 변수는 대부분 선언과 동시에 값을 할당한다.
...
10. enum 클래스에 선언되어 있지는 않지만 컴파일 시 자동으로 추가되는 상수의 목록을 배열로 리턴하는 메소드는 무엇인가요?
    - `values()` 메소드
...
1. String 클래스는 final 클래스인가요? 만약 그렇다면, 그 이유는 무엇인가요?
    - public final 클래스로 선언되어 있다.
    - 더 이상 확장해서는 안되기 때문이다.


2. String 클래스가 구현한 인터페이스에는 어떤 것들이 있나요?
    - String 클래스는 Serializable, Comparable, CharSequence 인터페이스를 구현했다.


3. String 클래스의 생성자 중에서 가장 의미없는 (사용할 필요가 없는) 생성자는 무엇인가요?
    - `new String()`
    - 이유 : 생성된 객체는 해당 변수에 새로운 값이 할당되자마자 GC의 대상이 되어버리기 때문이다.
...
7. String 클래스의 equals() 메소드와 compareTo() 메소드의 공통점과 차이점은 무엇인가요?
    - 공통점은 두 개의 문자열을 비교한다는 점이고, 차이점은 리턴 값이 다르다는 점이다.
...
9. 문자열에 "한국"이라는 단어의 위치를 찾아내려고 할 때에는 String의 어떤 메소드를 사용해야 하나요?
    - `indexOf()`
    - 원하는 문자열이 존재하는지 확인하고 싶다면 `contains()`나 `matches()` 메소드를 사용해도 된다.
...
12. 문자열의 모든 공백을 *표시로 변환하려고 합니다. 어떤 메소드를 사용하는 것이 좋을까요?
    ...
    - 원본 문자열은 변경되지 않으므로 변경된 값을 사용하려면 해당 메소드의 리턴 값을 사용해야만 한다.
...
1. @Override 어노테이션의 용도는 무엇인가요?
    - 해당 메소드가 부모클래스의 메소드를 Override 했다는 것을 명시적으로 알린다.
...
3. @Deprecated 어노테이션의 용도는 무엇인가요?
    - 이미 만들어져 있는 클래스나 메소드가 더 이상 사용되지 않음을 알린다.
...
### Chpater 18. 이제 기본 문법은 거의 다 배웠으니 정리해 봅시다

### <자바의 신> 중간 점검 및 실습

1. 참조 자료형(Reference type) 과 기본 자료형(Primitive type)의 차이를 정리해주세요.
    - 참조 자료형은 기본 자료형을 제외한 모든 타입을 말한다. 모든 클래스는 참조 자료형이라고 생각하면 된다. new 생성자를 지정하여 객체를 생성한 사용한다.
    - 기본 자료형은 숫자형과 boolean 을 나타내기 위한 자료형을 의미한다.
...
2. 기본 자료형 8가지를 나열하고 각 타입의 특징을 정리해주세요.
    ...
    - 실수형 : float, double
    - 기타 : boolean
    - byte 는 정수형이며 비트 수는 8비트이다. 기본 값은 0이고, 값의 범위는 -2<sup>7</sup> ~ 2<sup>7</sup> -1 이다.
    - short 는 정수형이며 비트 수는 16비트이다. 기본 값은 0이고, 값의 범위는 -2<sup>15</sup> ~ 2<sup>15</sup> -1 이다.
    - int 는 정수형이며 비트 수는 32비트이다. 기본 값은 0이고, 값의 범위는 -2<sup>31</sup> ~ 2<sup>31</sup> -1 이다.
    - long 은 정수형이며 비트 수는 64비트이다. 기본 값은 0L이고, 값의 범위는 -2<sup>63</sup> ~ 2<sup>63</sup> -1 이다.
    - char 은 정수형이며 비트 수는 16비트이다. 기본 값은 '\u0000'이고, 값의 범위는 0 ~ 2<sup>16</sup> -1 이다.
    - float 은 실수형이며, 기본 값은 0.0f 이다.
    - double 은 실수형이며, 기본 값은 0.0d 이다.
    - boolean 의 기본 값은 false 이다.


3. 형 변환이란 무엇이고 왜 해야 하나요?
    - 형 변환이란 자료형을 다른 타입으로 변환하는 작업이다.
    - 형 변환을 하지 않으면 값이 바뀔 수 있기 때문에 해야 한다.


4. if 문의 용도는 무엇이며, if-else 와 if-else if 는 어떤 점이 다른지 정리해 주세요.
    - 변수들의 상태에 따라서 프로그램을 분기할 필요가 있을 때 사용한다.
    - if-else 는 if 절의 조건 판단 후 결과가 false 일 때 else 절을 실행 한다.
    - if-else if 는 else if 절로 다른 조건을 따져 처리한다.


5. switch-case 문의 용도를 정리해 주세요.
    - 비교 대상의 변수의 값을 여러 범위에 걸쳐서 비교되어야 할 때 사용한다.
...
7. 학생이라면 지금까지의 자신의 학점이나 등수를, 회사원이라면 지금까지의 고과를 String 배열에 넣고 출력하는 코드를 작성해주세요.
   ```java
   String[] scoreArray = {"3.87", "4.24", "3.79", "3.82", "3.72", "3.94", "3.43", "4.5"};
...
8. 생성자는 무엇을 하는 데 사용하는 것이며, 별도로 만들지 않아도 자동으로 생성되는 생성자에 대해서 정리해 주세요.
    - 생성자는 객체를 초기화하기 위해 사용된다.
    - 매개변수가 없는 생성자가 자동으로 생성된다. ex) `public ClassName()`


9. Overloading은 무엇인가요? `public void setData(int a)`라는 메소드를 원하시는 대로 Overloading 해주세요.
    - ```public void setData(String data)```
    - ```public void setData(int a, String data)```


10. 패키지를 원하는 위치와 이름을 지정할 떄의 유의점을 정리해 주세요.
    - 패키지는 package로 시작하며, 하위 패키지로 내려갈 때마다 `.`을 찍어준다.
    - 반드시 소스의 가장 첫 줄에 존재해야 한다.
    - 패키지 명은 자바 예약어를 사용하면 안된다.
    - 패키지 명은 소문자로 작성하는 것이 일반적이다.
    - 일반적인 패키지는 java나 javax로 시작하면 안된다.


11. 다른 패키지에 선언된 클래스를 사용하기 위한 import는 어디 위치에 선언해야 하며, static import는 무엇인지 정리해 주세요.
    - package 선언부와 class 선언부 사이에 위치해야 한다.
    - 다른 클래스에 static으로 선언되어 있는 접근 가능한 변수를 참조하려면 static import 를 사용한다.


12. 클래스란 무엇인가요? 다음의 단어들이 포함되는 문장을 작성하고, 주어진 단어의 의미도 같이 정리 해주세요(속성, 상태).
    - 클래스란 속성(행위)과 상태를 갖는 자바의 가장 작은 단위를 말한다.
    - "속성"은 클래스나 인스턴스 변수로, "행위"는 메소드를 표현할 수 있다.


13. 인터페이스, abstract 클래스, 클래스, Enum 클래스가 있는데 각각의 특징 및 다른 점을 정리해 주세요.
    - 인터페이스
        - 어떤 메소드가 존재해야 하는 지에 대한 선언만 되어 있다.
        - 절대 구현되어 있는 메소드가 있으면 안된다.
        - 인터페이스를 구현하는 클래스에서는 implements 를 사용하여 선언한다.
    - abstract 클래스
        - 구현되어 있는 메소드가 있어도 상관 없다.
        - abstract method가 1개 이상일 경우에 무조건 abstract class로 선언해야 한다.
        - abstract로 선언된 메소드는 절대로 구현되어 있어서는 안된다.
        - abstract를 확장하는 클래스에서는 extends를 사용하여 선언한다.
    - 클래스
        - 모든 메소드가 구현되어 있어야 한다.
    - Enum 클래스
        - 상수를 열거하기 위한 용도로 사용된다.
        - enum 클래스의 상수는 이름만 정의해도 된다.
        - 별도의 생성자를 만들어 각 상수의 값을 지정할 수 있다.
        - 모든 enum 클래스의 부모 클래스는 java.lang.Enum 클래스 뿐이다.
        - enum에 메소드를 만들어 기능 추가가 가능하다.


14. instanceof 라는 연산자의 용도를 정리해 주세요.
    - 형변환을 하기 전에 타입을 확인하기 위한 용도로 사용한다.


15. 어떤 클래스를 상속받아 확장을 하면, 부모 클래스의 어떤 것들을 사용할 수 있는지 정리해 주세요.
    - public, protected로 선언된 변수와 메소드를 사용할 수 있다.
...
20. try-catch-finally 블록은 왜 사용하고 각각의 블록이 어떤 용도로 사용되는지 정리해 주세요.
    ...
    - catch : 예외를 처리한다.
    - finally : 예외가 발생하든 발생하지 않든 무조건 실행된다.